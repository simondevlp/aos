...
BOOTLOADER_IMAGE_PATH = $(DEBUG_BIN_PATH)/aos-bootloader.efi
KERNEL_IMAGE_PATH = $(DEBUG_BIN_PATH)/aos-kernel.efi
DEBUG_BIN_PATH = target/x86_64-unknown-uefi/debug
IMG_NAME = aos-uefi.img
OVMF_FD_PATH = /usr/share/ovmf/OVMF.fd

build:
	cargo build -p aos-bootloader --target x86_64-unknown-uefi
	cargo build -p aos-kernel --target x86_64-unknown-uefi
...
dir: build
	...
	mkdir -p $(ESP_DIR_NAME)/aos
	cp $(BOOTLOADER_IMAGE_PATH) $(BOOT_DIR_NAME)/bootx64.efi
	cp $(KERNEL_IMAGE_PATH) $(ESP_DIR_NAME)/aos/kernel.efi
